...
public class TSprite
{
	...
	protected virtual void UpdateVertices()
	{
		if (m_Rotation == 0f)
		{
			Rect rect = new Rect(m_Position.x - m_Scale * m_Size.x / 2f, m_Position.y - m_Scale * m_Size.y / 2f, m_Scale * m_Size.x, m_Scale * m_Size.y);
		...
		{
			float num = m_Scale * m_Size.x / 2f;
			float num2 = m_Scale * m_Size.y / 2f;
