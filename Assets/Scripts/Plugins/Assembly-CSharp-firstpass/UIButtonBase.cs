...
public class UIButtonBase : UIControlVisible
{
	...
	protected TUISprite m_HoverSprite;
	...
	public void SetAlpha(float alpha)
	{
		...
		SetSpriteAlpha(1, alpha);
		SetSpriteAlpha(2, alpha);
	}

	public float GetAlpha()
	{
		return GetSpriteAlpha((int)m_State);
	}
	...
	public Material GetTexture(State state)
	{
		return m_Sprite[(int)state].Material;
	}

	public Rect GetTextureRect(State state)
	{
		return m_Sprite[(int)state].TextureRect;
	}

	public Vector2 GetTextureSize(State state)
	{
		return m_Sprite[(int)state].Size;
	}
